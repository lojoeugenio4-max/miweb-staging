...
export default function BingoArticulos() {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      console.error("Error cargando artículos de bingo:", err);
  ...
  const articulosFiltrados = useMemo(() => {
    ...
    return articulos.filter((articulo) => {
      ...
      const coincideVista =
        vistaFiltro === "TODOS" ||
        (vistaFiltro === "EN_BINGO" && seleccionado) ||
        (vistaFiltro === "FUERA_BINGO" && !seleccionado);
  ...
  async function cambiarEstadoBingo(articulo, seleccionadoActual) {
    const codigo = String(articulo.codigo || "").trim();
  ...
  return (
    <div style={contenedor}>
      <h4 style={titulo}>📦 Artículos que cuentan para el Bingo</h4>
        ...
        <div style={filtros}>
          ...
          <label style={label}>
            ...
            >
              ...
              <option value="EN_BINGO">Solo en Bingo</option>
              <option value="FUERA_BINGO">Solo fuera de Bingo</option>
            </select>
          </label>
        </div>
                      ...
                      <td style={tdCentro}>
                        <label style={switchLabel}>
                          <input
                            ...
                            onChange={() => cambiarEstadoBingo(articulo, seleccionado)}
                          />
