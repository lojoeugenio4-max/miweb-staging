...
const TOTAL_DRAW_MS = (MIX_SECONDS + STOP_SECONDS + EXTRACTION_SECONDS + LANDING_PAUSE_SECONDS) * 1000 + 3900;
...
export default function BingoShow() {
  ...
  async function drawFromQr() {
    ...
    setDrawing(true);
    setError("");
    setDrawMessage("Validando participación de Bingo…");

    try {
      const { data: raw, error: drawError } = await supabase.rpc("consume_game_bingo_play_by_code", {
        p_code: qrCode,
      });
      const result = Array.isArray(raw) ? raw[0] : raw;

      if (drawError || !result?.ok) {
        throw new Error(result?.message || drawError?.message || "No se pudo extraer la bola.");
      }

      const ballNumber = Number(result.ball_number);
      if (!Number.isInteger(ballNumber) || ballNumber < 1 || ballNumber > BALL_COUNT) {
        throw new Error("Supabase no devolvió una bola válida.");
      }

      // La animación se inicia con la respuesta de la RPC. No dependemos de que
      // Realtime esté publicado/configurado para que el botón funcione.
      locallyStartedDraws.current.add(ballNumber);
      revealNumber(ballNumber);
      setDrawMessage(`Bola ${ballNumber} en juego…`);
      ...
      setDrawing(false);
      setDrawMessage("");
      setError(drawFailure?.message || "No se pudo extraer la bola.");
    }
  }
        ...
        <aside className="pro-presentation">
          ...
          {qrCode && !demoMode && (
            <div className="pro-draw-control">
              ...
              <span>{drawMessage || "El bombo está en espera. La bola solo saldrá al pulsar el botón."}</span>
            </div>
          )}
