...
function DrumCanvas({ moving, phase }) {
  ...
  useEffect(() => {
    ...
    function draw(now) {
      ...
      const speed = active ? 0.042 : 0.0035;
      rotation += speed * dt;
      ...
      balls.forEach((ball, index) => {
        if (active) {
          const swirl = 0.00055 + (index % 7) * 0.000025;
          ball.vx += -ball.y * swirl * dt;
          ball.vy += ball.x * swirl * dt;
          ball.vx += (Math.sin(rotation * 2.7 + index) * 0.00035) * dt;
          ball.vy += (Math.cos(rotation * 2.2 + index * 0.7) * 0.00035) * dt;
        } else {
          ball.vy += 0.00018 * dt;
          ball.vx *= 0.992;
          ball.vy *= 0.992;
      ...
      // Drum hoops and rotating cage ribs.
      ctx.save();
      ...
      for (let i = 0; i < 18; i += 1) {
        const angle = rotation + (i / 18) * Math.PI;
        const widthFactor = Math.abs(Math.cos(angle));
        ctx.beginPath();
        ctx.ellipse(cx, cy, Math.max(rx * 0.055, rx * widthFactor), ry * 0.95, 0, 0, Math.PI * 2);
        ctx.stroke();
      }
      ctx.globalAlpha = 0.75;
      ctx.lineWidth = Math.max(1.5, radius * 0.007);
      for (let i = 0; i < 7; i += 1) {
        const y = cy - ry * 0.72 + i * (ry * 1.44 / 6);
        const factor = Math.sqrt(Math.max(0, 1 - ((y - cy) / ry) ** 2));
        ctx.beginPath();
        ctx.ellipse(cx, y, rx * 0.93 * factor, Math.max(2, ry * 0.035), 0, 0, Math.PI * 2);
        ctx.stroke();
      }
      ctx.restore();
...
export default function BingoShow() {
  ...
  const revealNumber = useCallback((number) => {
    ...
    schedule(() => setPhase("extracting"), (MIX_SECONDS - 1.25) * 1000);
    schedule(() => {
  ...
  const moving = ["starting", "spinning", "extracting"].includes(phase);
  const status = phase === "starting" ? "MOTOR EN MARCHA" : phase === "spinning" ? "MEZCLANDO LAS 90 BOLAS" : phase === "extracting" ? "EXTRAYENDO BOLA" : phase === "reveal" ? "NUEVA BOLA" : displayNumber ? "ÚLTIMA BOLA CANTADA" : "TODO PREPARADO";
  ...
  return (
    <main className={`pro-stage pro-stage--${phase}`}>
      ...
      <section className="pro-content">
        <div className={`pro-machine ${moving ? "is-running" : ""}`}>
          <div className="pro-machine__backlight" />
          <div className="pro-frame">
            <div className="pro-frame__arch"><i/><i/><i/><i/><i/><i/><i/></div>
            ...
            <DrumCanvas moving={moving} phase={phase} />
            <div className={`pro-chute ${phase === "extracting" || phase === "reveal" ? "is-active" : ""}`}>
              <div className="pro-chute__mouth" />
              <div className="pro-chute__glass"><i>{pendingNumber}</i></div>
              <div className="pro-chute__tray"><span>{phase === "reveal" ? pendingNumber : ""}</span></div>
            </div>
        ...
        <aside className="pro-presentation">
          ...
          <div className={`pro-ball ${moving ? "is-mixing" : ""} ${phase === "reveal" ? "is-revealed" : ""}`}>
